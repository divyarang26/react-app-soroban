...
export default function Vault_req() {
  const invokerKeypair = SorobanClient.Keypair.fromSecret(
    process.env.REACT_APP_PRIVATE_KEY_BORROW
  );
  ...
  const handleClick = async (e) => {
    ...
    const server = new SorobanClient.Server(
      ...
      { allowHttp: true }
    ); // we call node as server here
    console.log('🚀 ~ file: client.js ~ line 69 ~ main ~ server', server);

    console.log(
      '🚀 ~ file: client.js ~ line 73 ~ main ~ invokerKeypair.publicKey()',
      invokerKeypair.publicKey()
    );

    let { sequence } = await server.getAccount(invokerKeypair.publicKey());
    console.log('🚀 ~ file: client.js ~ line 72 ~ main ~ sequence', sequence);

    let invokerAccount = new SorobanClient.Account(
      invokerKeypair.publicKey(),
      sequence
    ); // In stellar-sdk: // const invokerAccount = await server.loadAccount(invokerKeypair.publicKey());
    console.log(
      '🚀 ~ file: client.js ~ line 74 ~ main ~ invokerAccount',
      invokerAccount
    );
    ...
    let contractIdObj = SorobanClient.xdr.ScVal.scvObject(
      SorobanClient.xdr.ScObject.scoBytes(
        Buffer.from(process.env.REACT_APP_CONTRACT_ID, 'hex')
      )
    );
    ...
    contractMethodParameterObj = SorobanClient.xdr.ScVal.scvObject(
      SorobanClient.xdr.ScObject.scoU32(
        new SorobanClient.xdr.Uint32(Number(100))
      )
    ); // u64 - working ref
    ...
    opts.parameters.push(contractMethodParameterObj); // 4. Push Function Argument 2
    contractMethodParameterObj = SorobanClient.xdr.ScVal.scvObject(
      SorobanClient.xdr.ScObject.scoU32(new SorobanClient.xdr.Uint32(Number(8)))
    ); // u64 - working ref
    opts.parameters.push(contractMethodParameterObj); // 4. Push Function Argument 3
    contractMethodParameterObj = SorobanClient.xdr.ScVal.scvObject(
      SorobanClient.xdr.ScObject.scoU32(new SorobanClient.xdr.Uint32(Number(3)))
    ); // u64 - working ref
    opts.parameters.push(contractMethodParameterObj); // 4. Push Function Argument 4

    // // We'll add footprint in tx below, using way 01

    let hello_transaction = new SorobanClient.TransactionBuilder(
      invokerAccount,
      {
        fee: 100,
      ...
      .build();
    console.log(
      '🚀 ~ file: client.js ~ line 185 ~ main ~ build hello_transaction without footprint, only opts',
      hello_transaction
    );
    ...
    console.log(
      '🚀 ~ file: client.js ~ line 131 ~ main ~ Transaction Simulated; footprint',
      footprint
    );
    ...
    let transaction_with_footprint = addFootprint(
      hello_transaction,
      process.env.REACT_APP_NETWORK_PASSPHRASE,
      footprint
    );
    console.log(
      '🚀 ~ file: client.js ~ line 196 ~ main ~ transaction_with_footprint',
      transaction_with_footprint
    );
    ...
    try {
      ...
      console.log(
        `transaction_with_footprint was successfully submitted.\nComplete response: ${JSON.stringify(
          response
  ...
  return (
    <div style={{ paddingTop: '12px' }}>
      <div className='card'>
        <h1 className='card-header'>Vault Request</h1>
